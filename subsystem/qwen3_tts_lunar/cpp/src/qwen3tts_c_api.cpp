...
static qwen3_tts::tts_params to_cpp_params(const Qwen3TtsParams * p) {
    ...
    qwen3_tts::set_ggml_cpu_threads(p ? p->n_threads : 4);
    return params;
}
...
static bool c_stream_wrapper(const float * samples, int32_t n_samples,
                              int32_t sample_rate, bool is_final,
                              streaming_ctx * ctx) {
    if (ctx->aborted) return false;
    int ret = ctx->callback(samples, n_samples, sample_rate, is_final, ctx->user_data);
    if (ret != 0) {
        ctx->aborted = true;
        return false;
    }
...
int qwen3_tts_synthesize_streaming(
    ...
    if (!tts || !text || !reference_audio_path || !callback) return 1;
    AUTORELEASE_BEGIN
    auto cpp_params = to_cpp_params(params);
    ...
    auto cpp_callback = [&](const float * samples, int32_t n_samples,
                            int32_t sample_rate, bool is_final) -> bool {
        return c_stream_wrapper(samples, n_samples, sample_rate, is_final, &ctx);
    };

    bool ok = tts->engine.synthesize_streaming(
        text, reference_audio_path, cpp_params, cpp_callback, chunk_frames);
    if (!ok) {
        tts->last_error = tts->engine.get_error();
    }
    AUTORELEASE_END
...
int qwen3_tts_synthesize_streaming_with_embedding(
        ...
        void * user_data,
        int32_t chunk_frames) {
    if (!tts || !text || !embedding || embedding_size <= 0 || !callback) return 1;
    AUTORELEASE_BEGIN
    auto cpp_params = to_cpp_params(params);
    ...
    bool ok = tts->engine.synthesize_streaming_with_embedding(
        text, embedding, embedding_size, cpp_params, cpp_callback, chunk_frames);
    if (!ok) {
        tts->last_error = tts->engine.get_error();
    }
    AUTORELEASE_END
