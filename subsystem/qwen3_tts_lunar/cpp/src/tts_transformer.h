...
public:
    ...
    // Frame callback type for streaming generation
    // Called after each frame is generated with [n_codebooks] codes
    // Return true to continue, false to abort
    using frame_callback_t = std::function<bool(const int32_t* frame_codes, int32_t frame_index)>;

    // Generate speech codes with per-frame callback for streaming
