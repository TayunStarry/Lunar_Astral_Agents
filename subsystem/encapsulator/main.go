...
func main() {
	...
	if err := code.LoadPackageConfig(*code.ConfigPath); err != nil {
	...
	// 获取内置的默认配置值
	defaultOut, defaultPartSize, defaultCompress, defaultPackage := code.GetDefaultConfig()
	// 若用户未指定 output_path，则使用默认值
	if !specified["output_path"] {
		*code.OutputPath = defaultOut
	}
	// 若用户未指定 part_size_mb，则使用默认值
	if !specified["part_size_mb"] {
		*code.PartSizeMB = defaultPartSize
	}
	// 若用户未指定 compression_level，则使用默认值
	if !specified["compression_level"] {
		*code.CompressionLevel = defaultCompress
	}
	// 若用户未指定 package_level，则使用默认值
	if !specified["package_level"] {
		*code.PackageLevel = defaultPackage
	}
	// 若已设置输出路径且分卷大小不为零，则执行打包流程
	if *code.OutputPath != "" && *code.PartSizeMB != 0 {
		code.ExecutePackageProcess()
		return
	}
	// 否则输出错误并退出程序
