...
// samples: PCM float32 mono normalized [-1,1]
// n_samples: number of samples in this chunk
// sample_rate: always 24000
// is_final: true if this is the last chunk
// Returns true to continue, false to abort
using tts_stream_callback_t = std::function<bool(const float* samples, int32_t n_samples,
...
public:
    ...
    // Synthesize with streaming PCM output via callback
    // text: input text to synthesize
    // reference_audio: path to reference WAV file
    // params: generation parameters
    // callback: invoked for each decoded PCM chunk
    // chunk_frames: accumulate this many frames before callback (0=default ~50 frames ≈ 1s)
    // Returns true on success, false on failure or callback abort
    ...
    // Synthesize with streaming PCM output and pre-computed embedding (skips encoder)
    // text: input text to synthesize
    // embedding: speaker embedding from extract_speaker_embedding()
    // embedding_size: must match hidden_size (typically 1024)
    // params: generation parameters
    ...
    // chunk_frames: accumulate this many frames before callback (0=default ~50 frames ≈ 1s)
    // Returns true on success, false on failure or callback abort
    bool synthesize_streaming_with_embedding(const std::string & text,
...
private:
    ...
    tts_stream_callback_t stream_callback_;
};
