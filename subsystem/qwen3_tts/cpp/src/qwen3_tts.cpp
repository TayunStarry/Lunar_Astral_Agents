...
static bool get_process_memory_snapshot(process_memory_snapshot & out) {
    ...
    out.phys_footprint_bytes = (uint64_t) pmc.PagefileUsage;
    return true;
#else
    struct rusage usage = {};
...
bool Qwen3TTS::load_models(const std::string & model_dir) {
    ...
    low_mem_mode_ = low_mem_env && (low_mem_env[0] == '1' || low_mem_env[0] == 'y' || low_mem_env[0] == 'Y');
    if (low_mem_mode_) {
        fprintf(stderr, "  [WARN] Low-memory mode enabled (lazy decoder + component unloads)\n");
    } else {
        fprintf(stderr, "  Server mode: models kept resident for fast response\n");
    }
...
bool Qwen3TTS::synthesize_streaming(const std::string & text,
                                     const std::string & reference_audio,
                                     const tts_params & params,
                                     tts_stream_callback_t callback,
    ...
    frame_buffer.reserve(chunk_frames * n_codebooks);
    int32_t total_frames_decoded = 0;
    bool aborted = false;

    auto frame_cb = [&](const int32_t* codes, int32_t frame_idx) -> bool {
        for (int cb = 0; cb < n_codebooks; ++cb) {
            frame_buffer.push_back(codes[cb]);
        }

        // Check if we have enough frames to decode a chunk
        int frames_in_buf = (int)frame_buffer.size() / n_codebooks;
        if (frames_in_buf >= chunk_frames) {
            // Decode accumulated frames
            int frames_to_decode = frames_in_buf;
            std::vector<float> pcm_chunk;
            if (!audio_decoder_.decode(frame_buffer.data(), frames_to_decode, pcm_chunk)) {
                error_msg_ = "Failed to decode speech chunk: " + audio_decoder_.get_error();
                aborted = true;
                return false;
            }

            // Send PCM chunk via callback
            if (!callback(pcm_chunk.data(), (int32_t)pcm_chunk.size(),
                          audio_decoder_.get_config().sample_rate, false)) {
                aborted = true;
                return false;
            }

            total_frames_decoded += frames_to_decode;
            frame_buffer.clear();
        }
        return true;
    };
    ...
    bool ok = transformer_.generate_streaming(
        text_tokens.data(), (int32_t)text_tokens.size(),
        speaker_embedding.data(), params.max_audio_tokens,
        frame_cb, params.language_id, params.repetition_penalty,
        params.temperature, params.top_k);
    ...
    // Decode remaining frames (final chunk)
    if (!aborted && !frame_buffer.empty()) {
        int frames_remaining = (int)frame_buffer.size() / n_codebooks;
        std::vector<float> final_pcm;
        if (!audio_decoder_.decode(frame_buffer.data(), frames_remaining, final_pcm)) {
            error_msg_ = "Failed to decode final chunk: " + audio_decoder_.get_error();
            return false;
        }

        if (!callback(final_pcm.data(), (int32_t)final_pcm.size(),
                      audio_decoder_.get_config().sample_rate, true)) {
            return false;
        }
    } else if (!aborted) {
        // Signal completion even if no remaining frames
        float dummy = 0.0f;
        callback(&dummy, 0, audio_decoder_.get_config().sample_rate, true);
    }

    return !aborted;
}

bool Qwen3TTS::synthesize_streaming_with_embedding(const std::string & text,
    ...
    // Run streaming generation with pre-computed embedding
    bool ok = transformer_.generate_streaming(
        text_tokens.data(), (int32_t)text_tokens.size(),
        const_cast<float*>(embedding), params.max_audio_tokens,
        frame_cb, params.language_id, params.repetition_penalty,
        params.temperature, params.top_k);
