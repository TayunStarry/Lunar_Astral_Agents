...
static void compute_mel_filterbank_slaney(float * filterbank, int n_mels, int n_fft,  int sample_rate, float f_min, float f_max) {
...
bool AudioTokenizerEncoder::load_model(const std::string & model_path) {
    ...
    bool is_gpu = device && ggml_backend_dev_type(device) != GGML_BACKEND_DEVICE_TYPE_CPU;
    fprintf(stderr, "  [Backend] Active: %s [%s]\n", device_name, is_gpu ? "GPU Acceleration" : "CPU Only");
