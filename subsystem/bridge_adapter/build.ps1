...
# ---------- 构建主流程 ----------
function Build-BridgeAdapter {
    Write-Host "`n========== 构建 bridge_adapter ==========" -ForegroundColor Magenta
...
try {
    if (-not $SkipCheck) {
        Write-Host "[bridge_adapter] 独立编译模式，执行环境检查..." -ForegroundColor Cyan
        Test-GoCommand
    }
    else {
        Write-Host "[bridge_adapter] 由主调度脚本调用，跳过环境检查。" -ForegroundColor Yellow
    }
    Build-BridgeAdapter
}
catch {
