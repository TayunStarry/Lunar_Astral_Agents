...
try {
    ...
    $binaryName = "bridge_adapter.exe"
    if ($TargetOS -ne "windows") { $binaryName = "bridge_adapter" }
    ...
    Write-Host "✓ bridge_adapter 构建成功: $outputPath" -ForegroundColor Green
}
catch {
    Write-Host "`n[ERROR] bridge_adapter 构建失败: $_" -ForegroundColor Red
    exit 1
}
