...
// permsHandler GET /api/perms 返回权限名全集（前端下拉框，allow-*）。
...
// 绑定代码文件为打包后的单一 execute.js，来源二选一（dir 优先）：
//   - multipart 文件上传：字段名 file（execute.js）+ 表单字段 permissions（JSON 数组）
//   - 表单字段 dir：插件包目录绝对路径（后端自行读取 execute.js，保证与引擎哈希规则一致）
func genHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 绑定代码文件来源：目录优先，其次单个 execute.js 上传
	var code []byte
	if dir := strings.TrimSpace(r.FormValue("dir")); dir != "" {
		...
		_, data, err := readExecuteJSFromDir(dir)
		if err != nil {
			jsonOK(w, http.StatusInternalServerError, &genResponse{Success: false, Error: err.Error()})
			return
		}
		code = data
	} else {
		fh, header, ferr := r.FormFile("file")
		if ferr != nil {
			jsonOK(w, http.StatusBadRequest, &genResponse{Success: false, Error: "请上传 execute.js 文件或填写插件目录"})
			return
		}
		defer fh.Close()
	...
	if len(code) == 0 {
		jsonOK(w, http.StatusBadRequest, &genResponse{Success: false, Error: "execute.js 内容为空"})
		return
	}

	// 生成密钥：每权限一条 32 字符密钥字符串，整体加密为一段密文
	hashKey := codeHashFromData(code)
	resp.Hash = hashKey
