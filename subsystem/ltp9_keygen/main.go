...
// ==== LTP9 权限密钥生成器入口 ====
// 根开发者在 Web 前端选择权限、导入 execute.js（或填写插件目录），
// 生成 permissions.key 写入插件包，供 LTP9 引擎加载时校验授权。
...
func main() {
	...
	// webview 窗口尺寸（密钥生成界面按较大尺寸布局）
	*GeneralConfig.WebViewTitle = "LTP9 权限密钥生成器"
	...
	*GeneralConfig.WebViewResizable = true
	...
	go func() {
		LoggerGeneral.Info("LTP9Keygen", "密钥生成器服务已启动: http://%s", addr)
		if err := server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			LoggerGeneral.Error("LTP9Keygen", "服务运行失败: %v", err)
		}
	}()
	...
	select {
	case <-quit:
		LoggerGeneral.Info("LTP9Keygen", "收到中断信号，正在关闭...")
	case <-BrowserClient.WebViewClosed():
		LoggerGeneral.Info("LTP9Keygen", "窗口已关闭，正在退出...")
	}
