...
`logger_general` 模块是星月智能平台的**基础设施层**，为所有子系统提供统一的彩色终端日志输出。
...
| 日志文件 | 开发模式下同时写入控制台与日志文件（自动剥离 ANSI 颜色码） |
...
<div style="font-family: 'Cascadia Code', 'SF Mono', Consolas, monospace; font-size: 0.9em; line-height: 1.6;">
  <ul style="list-style-type: none; padding-left: 0;">
    <li><strong>logger_general/</strong></li>
    <li style="padding-left: 1.5em;"><code>logger.go</code> <span style="color: #6a737d;">— 彩色终端日志输出（分级日志 + ANSI 颜色 + 开发模式控制）</span></li>
    <li style="padding-left: 1.5em;"><code>variable.go</code> <span style="color: #6a737d;">— 全局变量与颜色常量</span></li>
...
```
SetDevMode(true, "logs")    ← 开启开发模式（日志文件保存到 logs/ 目录）
    │
    ├── Info / SubInfo → 输出到控制台 ✓ + 写入日志文件
    ├── Warn / SubWarn → 输出到终端 ✓
    ├── Error / SubError → 输出到终端 ✓
    └── Fatal → 输出到终端 + os.Exit(1) ✓

SetDevMode(false, "")       ← 关闭开发模式（默认，仅控制台输出）
    │
    ├── Info / SubInfo → 静默 ✗
    ├── Warn / SubWarn → 输出到终端 ✓
    ├── Error / SubError → 输出到终端 ✓
...
> 开发模式下日志文件名采用「程序启动时间.log」，写入时自动剥离 ANSI 颜色码，便于直接查看。日志文件目录可配置，为空则保存到当前目录。
...
| 函数 | 说明 |
|------|------|
| `SetDevMode(v bool, logDir string)` | 设置开发模式开关与日志文件保存目录（Info 级别仅在开发模式下输出） |
...
func main() {
    // 启用开发模式，日志文件保存到 logs/ 目录
    LoggerGeneral.SetDevMode(true, "logs")

    // 各级别日志输出
    LoggerGeneral.Info("MyModule", "服务启动于端口 %d", 8080)
    LoggerGeneral.Warn("MyModule", "配置项缺失，使用默认值: %s", "timeout")
    LoggerGeneral.Error("MyModule", "连接失败: %v", err)
    LoggerGeneral.Fatal("MyModule", "无法加载模型: %v", err)  // 输出后 os.Exit(1)
}
```
...
```go
LoggerGeneral.SubInfo("ProxySvr", "Cert", "从磁盘加载证书: %s", certPath)
LoggerGeneral.SubWarn("ProxySvr", "TLS", "证书即将过期: %s", expiryDate)
LoggerGeneral.SubError("ProxySvr", "WebSocket", "Hijack 失败: %v", err)
```
...
```go
// 在程序入口处，根据配置设置开发模式
LoggerGeneral.SetDevMode(*config.Developer, *config.LogDir)
```
