...
func VideoKeyframeExtraction(inputFile string) ([]KeyFrame, error) {
	...
	LoggerGeneral.Info("MultimodalAnalysis", "开始提取关键帧，视频时长: %.1f秒，提取频率: %.0ffps", duration, DefaultFPS)
	...
	LoggerGeneral.Info("MultimodalAnalysis", "帧提取完成，共 %d 帧，耗时: %v", len(allFrames), extractElapsed)
	...
	for i, currImage := range allFrames {
		...
		if err != nil {
			LoggerGeneral.Error("MultimodalAnalysis", "创建关键帧文件失败(帧%d): %v", i, err)
			continue
		}
	...
	LoggerGeneral.Info("MultimodalAnalysis", "关键帧筛选完成，从 %d 帧中保留 %d 帧，总耗时: %v",
...
func extractAllFramesAtFPS(inputFile string, fps float64) ([]image.Image, error) {
	...
	for len(data) > 0 {
		...
		if err != nil {
			LoggerGeneral.Warn("MultimodalAnalysis", "解码帧%d失败（跳过）: %v", frameCount, err)
		} else {
