...
static void ensure_enc_fwd_buffers(qwen_encoder_t *enc, int total_tokens, int d_model, int ffn_dim) {
    if (total_tokens > enc->fwd_cap_tokens) {
        enc->fwd_cap_tokens = total_tokens;
        enc->fwd_cap_ffn_dim = ffn_dim;
        enc->fwd_cap_proj_mid = total_tokens;
        enc->fwd_x      = (float *)realloc(enc->fwd_x,      (size_t)total_tokens * d_model * sizeof(float));
        enc->fwd_x_norm = (float *)realloc(enc->fwd_x_norm, (size_t)total_tokens * d_model * sizeof(float));
        enc->fwd_q      = (float *)realloc(enc->fwd_q,      (size_t)total_tokens * d_model * sizeof(float));
        enc->fwd_k      = (float *)realloc(enc->fwd_k,      (size_t)total_tokens * d_model * sizeof(float));
        enc->fwd_v      = (float *)realloc(enc->fwd_v,      (size_t)total_tokens * d_model * sizeof(float));
        enc->fwd_attn_out = (float *)realloc(enc->fwd_attn_out, (size_t)total_tokens * d_model * sizeof(float));
        enc->fwd_proj_out = (float *)realloc(enc->fwd_proj_out, (size_t)total_tokens * d_model * sizeof(float));
        enc->fwd_ffn_mid  = (float *)realloc(enc->fwd_ffn_mid, (size_t)total_tokens * ffn_dim * sizeof(float));
        enc->fwd_ffn_out  = (float *)realloc(enc->fwd_ffn_out, (size_t)total_tokens * d_model * sizeof(float));
        enc->fwd_proj_mid = (float *)realloc(enc->fwd_proj_mid, (size_t)total_tokens * d_model * sizeof(float));
    }
}
...
float *qwen_encoder_forward(qwen_ctx_t *ctx, const float *mel, int mel_frames,
    ...
    int chunk_size = cfg->enc_chunk_size;
    int n_window_infer = cfg->enc_n_window_infer;
    ...
    int total_tokens = 0;
    int *chunk_offsets = (int *)malloc((size_t)(n_chunks + 1) * sizeof(int));
    int *chunk_w3 = (int *)malloc((size_t)n_chunks * sizeof(int));
    ...
    for (int c = 0; c < n_chunks; c++) {
        ...
        int w2 = (w1 + 2 - 3) / 2 + 1;
        int w3 = (w2 + 2 - 3) / 2 + 1;
        chunk_offsets[c] = total_tokens;
        total_tokens += w3;
    }
    chunk_offsets[n_chunks] = total_tokens;

    ensure_enc_fwd_buffers(enc, total_tokens, d_model, ffn_dim);

    float *x = enc->fwd_x;
    memset(x, 0, (size_t)total_tokens * d_model * sizeof(float));
    ...
    for (int c = 0; c < n_chunks; c++) {
        ...
        for (int m = 0; m < 128; m++)
            memcpy(chunk_mel + m * chunk_w,
                   mel + m * mel_frames + start,
    ...
    /* ---- Transformer layers ---- */
    float *x_norm = enc->fwd_x_norm;
    float *q = enc->fwd_q;
    float *k = enc->fwd_k;
    float *v = enc->fwd_v;
    float *attn_out = enc->fwd_attn_out;
    float *proj_out = enc->fwd_proj_out;
    float *ffn_mid = enc->fwd_ffn_mid;
    float *ffn_out = enc->fwd_ffn_out;
    ...
    float *proj_mid = enc->fwd_proj_mid;
