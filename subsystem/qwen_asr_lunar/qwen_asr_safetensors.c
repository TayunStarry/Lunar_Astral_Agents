...
safetensors_file_t *safetensors_open(const char *path) {
    ...
    if (fileSize < 8) { CloseHandle(hFile); return NULL; }
...
multi_safetensors_t *multi_safetensors_open(const char *model_dir) {
    ...
    snprintf(searchPath, sizeof(searchPath), "%s\\*.safetensors", model_dir);
    HANDLE hFind = FindFirstFileA(searchPath, &findData);
    if (hFind == INVALID_HANDLE_VALUE) { free(ms); return NULL; }

    char shard_names[SAFETENSORS_MAX_SHARDS][256];
    int n_shards = 0;

    do {
        /* Accept any .safetensors file, not just model-* */
        const char *fname = findData.cFileName;
        size_t len = strlen(fname);
        if (len > 12 && strcmp(fname + len - 12, ".safetensors") == 0) {
            snprintf(shard_names[n_shards], sizeof(shard_names[n_shards]),
                     "%s", fname);
            n_shards++;
        }
    } while (FindNextFileA(hFind, &findData) != 0 && n_shards < SAFETENSORS_MAX_SHARDS);
    FindClose(hFind);
#else
