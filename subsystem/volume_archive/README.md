...
打包时自动过滤以下类型的文件/目录，匹配逻辑基于 `filepath.Match` 通配符（目录规则以 `/` 结尾，仅匹配目录）：

| 类别 | 规则模式 | 说明 |
|------|---------|------|
| **构建产物** | `dist/`, `build/` | 排除构建输出目录 |
| **版本控制** | `.git/` | 排除 Git 仓库 |
| **依赖目录** | `node_modules/`, `__pycache__/`, `*.egg-info/` | 排除第三方依赖 |
| **编译/缓存文件** | `*.pyc`, `*.pyo`, `*.ts`, `*.go` | 排除源代码和编译中间文件 |
| **临时文件** | `*.log`, `*.tmp`, `*.bak` | 排除临时和备份文件 |
| **系统文件** | `.DS_Store` | 排除 macOS 系统文件 |

### 文件列表生成机制

为避免 Windows 命令行参数长度限制，源文件路径先写入系统临时目录的 `7z_file_list.txt` 文件（每行一个相对路径），再通过 `@` 前缀传递给 7z：

```
源目录递归扫描 → 应用排除规则 → 相对路径列表 → 写入临时文件 → 7z a @临时文件
```

临时文件在命令执行后通过 `defer os.Remove` 自动清理。
...
```bash
7z a -v{size}m -mx={level} -mmt -bb1 -bso1 -bsp1 {output}.7z @filelist
```

| 参数 | 说明 | 示例 |
|------|------|------|
| `a` | 添加到压缩包 | — |
| `-v{size}m` | 分卷大小（MB），如 `-v2048m` | `-v1024m` |
| `-mx={level}` | 压缩级别 0-9 | `-mx=5` |
| `-mmt` | 启用多线程压缩 | — |
| `-bb1` | 日志级别 1（基本输出） | — |
| `-bso1` | 标准输出流启用 | — |
| `-bsp1` | 进度输出流启用（供进度条解析） | — |
| `@{file}` | 从文件读取输入路径列表 | `@C:\Temp\7z_file_list.txt` |

- **格式：** 7z（7-Zip 原生格式）
- **算法：** LZMA2（支持固实压缩、多线程、高压缩比）
- **工作目录：** 7z 在基准目录（第一个源的父目录）下执行，压缩包内使用相对路径

### 压缩级别详解

| 级别 | 含义 | 压缩方式 | 适用场景 |
|------|------|---------|---------|
| **0** | 不压缩（仅存储） | Copy | 已压缩文件（视频、图片），仅需分卷 |
| **1-3** | 快速 | Fast | 对速度要求高 |
| **4-6** | 正常 | Normal | **级别 5 为默认**，适合大多数场景 |
| **7-8** | 最大 | Maximum | 对压缩率要求高 |
| **9** | 极限 | Ultra | 固实压缩，最高压缩率，耗时最长 |
...
|------|---------|
| **700 MB** | CD 介质分发 |
| **2048 MB**（默认） | 大多数场景，兼容 FAT32 |
| **4096 MB** | 大容量存储，减少分卷数量 |
...
|---------|-------------------|
| 文本文件（`.txt`, `.json`） | 60-80% |
| 代码文件（`.js`, `.html`, `.css`） | 65-75% |
| 二进制文件（`.exe`, `.dll`） | 40-60% |
| 已压缩文件（`.zip`, `.jpg`, `.png`） | 0-5%（几乎不压缩） |

> 压缩比以原始大小为基准，数值越高压缩越有效。固实压缩下相似文件打包时压缩率更高。

### 输出分卷命名

```
{OutputPath}.7z.001    ← 第一卷（最多 part_size_mb MB）
{OutputPath}.7z.002    ← 第二卷
{OutputPath}.7z.003    ← 最后卷（剩余数据）
```

序号从 `001` 开始按三位数字递增，超过 999 自动扩展位数。除最后一卷外每卷精确等于指定大小。
...
| 模块 | 技术选型 | 说明 |
|------|---------|------|
| **配置管理** | JSON + flag | 配置文件定义参数，命令行覆盖 |
| **文件扫描** | `os.ReadDir` + 递归 | 目录遍历，排除规则过滤 |
| **压缩引擎** | 7-Zip CLI | 外部进程调用，分卷+多线程 |
| **进度显示** | 正则解析 + ANSI 颜色 | 实时解析 7z 输出，彩色进度条 |
| **路径处理** | `filepath` 包 | 绝对路径/相对路径计算 |
| **临时文件** | `os.TempDir` | 文件列表存入临时目录，自动清理 |
| **错误处理** | 警告继续 / 致命退出 | 扫描失败告警不中断，关键失败退出 |
...
2. **路径前缀：** 配置文件中的路径相对于项目根目录，需注意 `../../` 前缀（从 `subsystem/volume_archive` 上溯至项目根）
3. **分卷完整性：** 解压时需要所有分卷文件在同一目录
4. **文件系统限制：** FAT32 不支持超过 4GB 的分卷
5. **编码：** 文件名使用系统默认编码，非 UTF-8 环境可能有问题
6. **权限：** 需要临时目录和目标目录的写入权限
