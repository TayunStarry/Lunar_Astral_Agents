...
// 插件以 engine.command.register(name, pattern, handler, options) 注册「/xxx」类指令，
// 引擎侧经 Command(pluginID, text) 触发：按指令名/别名或首个匹配正则的处理器派发。
// 处理器签名 (match, context) => result，match 为捕获组数组，context 为可选上下文。
...
// Command 触发某插件指令（等价于 engine 侧分发）。text 匹配注册时的指令名/别名或正则；
