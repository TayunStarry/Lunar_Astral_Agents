...
### 1.4 权限密钥（permissions.key）

LTP3 采用**密钥授权**替代旧的 `plugin.json` 权限声明。插件包根目录需要一个由根开发者生成的 `permissions.key`：

- **生成**：使用独立程序 `subsystem/ltp3_keygen`（Web 界面：下拉选权限 + 拖入插件脚本）。它把全部 `.js` 脚本拼接 → SHA-256 取前 128 位作为密钥；**每个权限独立生成一条 32 字符密钥字符串**（权限名 + `/` 填充，10 个权限 → 10 条 32 字符密钥），以 `+`（取自 base64 字符集）连接成明文报文后，**用脚本哈希对整条报文整体加密**写入文件——密文中看不到分隔符与密钥边界（完全混淆）。
- **校验**：引擎加载插件时按同一规则重算脚本哈希，**先整体解码**拿到明文，**再按 `+` 分割**、去掉 `/` 填充恢复权限名；**整体解码失败或名称不在权限名单 → 视为密钥与脚本不对应/脚本被篡改，拒绝该权限**。
- **生效**：未授权 API 在沙箱中为 `undefined`；插件脚本若在顶层使用未授权 API 会加载失败，报错会提示缺少 `permissions.key` 或脚本被篡改。
- **开发模式豁免**：以 `-developer` 启动引擎时，会**跳过权限密钥校验、默认授予全部权限**——插件可无需 `permissions.key` 直接开发调试，改动脚本也不用重新生成密钥。仅开发环境使用，生产环境关闭该开关以启用鉴权。
- **注意**：脚本一旦改动，哈希即变，原密钥全部失效，需用密钥生成器重新生成。
