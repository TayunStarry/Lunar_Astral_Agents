...
// ==== 前端可视化测试探针接口（沙箱能力复用，供 ltp9/test 信封调用） ====
...
// ProbeMemory 记忆库读写。op: store|search|searchImage|storeImage|randomImage。
func ProbeMemory(op string, v map[string]any) map[string]any {
	...
	case "searchImage":
		...
		return probeResult(memorySearchImage(q, limit))
	case "storeImage":
		img, _ := v["image"].(string)
		return probeResult(memoryStoreImage(img))
	case "randomImage":
		return probeResult(memoryRandomImage(q))
	default: // store
		return probeResult(memoryStore(v))
	}
}

// ProbeEncode 加密（encoding.lunarEncoder 同源实现）。
func ProbeEncode(key, content string) map[string]any {
	return probeResult(encodingLunarEncoder(key, content))
}

// ProbeDecode 解密（encoding.lunarDecoder 同源实现）。
func ProbeDecode(key, cipher string) map[string]any {
	return probeResult(encodingLunarDecoder(key, cipher))
}

// ProbeJWT JWT 签名（hash.signJWT 同源实现）。
...
// ==== 网络探针（network 同源实现；套接字以句柄登记，供前端分步操作） ====
...
func ProbeAsync(op string, params map[string]any) map[string]any {
	...
	case "run":
		...
		go func() {
			var err error
			if pluginID != "" && fnName != "" {
				// 真实任务体：后台调用插件导出的函数（与 async.run 一致，任务运行期间占用该插件事件循环）
