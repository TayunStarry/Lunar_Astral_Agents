...
// SetOutbound 注入插件单向通报接收函数（signal.all/target 与 event.publish 的转发目标，供外部 Go 程序消费）。
...
// SetAgentInvoker 注入前端智能体调用实现（agent.synergy 的真实通道）。
// 由宿主（crystal_astral）提供：把 appID + 自然语言转交 Mini-LTP / Node-LTP 包并返回其执行文本。
// 未注入时，agent.synergy 返回「xxx 包拒绝响应」。
...
// Call 由外部 Go 程序直接调用某个插件导出的函数（等价于沙箱内 callFunction）。
