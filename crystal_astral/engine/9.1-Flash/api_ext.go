...
// ==== 扩展能力实现：JWT / 摘要 / Ed25519 / 图像 / LLM ====
// LLM 配置读取 lunar_config.json 的 agent 字段（GeneralConfig 同源），不硬编码。
...
// ==== JWT 签名（hash.signJWT，allow-certificate） ====
...
// ==== 摘要 / HMAC / Ed25519 / JWT 原语（hash.*，allow-certificate） ====
...
// ==== 图像处理 image.loadValid/download（allow-file：读/校验；allow-network：下载） ====
...
// image.loadValid 之外，下载另需 allow-network（契约：image 能力下载要求网络权限）。
func engineImageDownload(p *plugin, url, fileName string) (any, error) {
	if !p.hasPerm("allow-network") {
		return rwResult{Success: false, Error: "image.download 需要allow-network权限"}, nil
	}
...
// ==== LLM agent.chat/embed（allow-agent，读取 lunar_config.json 的 agent 字段） ====
...
// previewStr 把字节串截断为最多 n 字符的预览（超长以 ... 结尾）。
