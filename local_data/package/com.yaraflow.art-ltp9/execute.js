 ...
 * 网络走 engine.http.get/post；图片 engine.image；LLM engine.llm；发送 engine.send；休眠 engine.sleep。
...
// ===== 每日次数（本地日期零点重置，engine.file 同步持久化） =====

function today() { return new Date(engine.time.now() * 1000).toISOString().slice(0, 10); }
function loadDailyCount() {
  try {
    const r = engine.file.read("daily_count.json");
...
function persistDailyCount(r) { try { engine.file.write("daily_count.json", JSON.stringify(r)); } catch (e) {} }
...
function httpPostJson(url, body, headers) {
  const r = engine.http.post(url, JSON.stringify(body), headers || {});
...
function downloadRefBase64(url, index) {
  try {
    const r = engine.image.download(url, "ref_" + index + ".png");
    if (r && r.success && r.text) return r.text;
...
function optimizePrompt(description) {
  try {
    if (typeof engine.llm !== "object" || !engine.llm.chat) return { ok: false, prompt: description };
    const resp = engine.llm.chat([
  ...
  for (let i = 0; i < 24; i++) {
    ...
    if (s.status >= 200 && s.status < 300) {
      ...
      if (sd && sd.task_status === "SUCCEED") {
        ...
        if (url) { const dl = engine.image.download(url, "gen_" + taskId + ".png"); if (dl && dl.success && dl.text) return { ok: true, b64: dl.text }; return { ok: false, error: "魔搭图片下载失败" }; }
...
function callComfyui(cfg, prompt, size, inputB64) {
  ...
  const fr = engine.file.read("workflow/" + wfName);
...
function generatePicture(params, context) {
  ...
  try {
    ...
    let b64 = result.b64 || "";
    if (!b64 && result.url) { const dl = engine.image.download(result.url, "result.png"); if (dl && dl.success && dl.text) b64 = dl.text; }
    if (!b64) return { ok: false, text: log(true, "绘图失败: 未获取到图片数据") };

    let sent = false;
    if (groupId && typeof engine.send === "object" && engine.send.image) {
      const sr = engine.send.image(groupId, b64);
      sent = !!(sr && sr.success);
    }
    const remain = dailyMax - getDailyCount();
    if (sent) return { ok: true, b64: b64, text: log(false, "图片生成并发送成功（今日剩余 " + remain + " 次）") };
    return { ok: true, b64: b64, mime: guessMime(b64), text: log(false, "图片生成成功（未发送，b64长度=" + b64.length + "）") };
  } catch (e) {
...
function layerSplit(params, context) {
  ...
  try {
    ...
    const baseDl = engine.image.download(base.url, "layer_base.png");
    ...
    for (let j = 0; j < layers.length && j < 16; j++) {
      const l = layers[j];
      if (!l.url) continue;
      const ld = engine.image.download(l.url, "layer_" + j + ".png");
    ...
    let sentOk = true;
    if (groupId && typeof engine.send === "object" && engine.send.hybrid) {
      const msgs = []; msgs.push({ type: "text", content: summary });
      for (const s of segs) msgs.push(s);
      const sr = engine.send.hybrid(groupId, msgs);
      sentOk = !!(sr && sr.success);
    }
    const remain = dailyMax - getDailyCount();
    if (sentOk) return { ok: true, text: log(false, summary + "\n" + labels.join("\n") + "\n（今日剩余 " + remain + " 次）") };
    return { ok: true, segs: segs, text: log(false, "拆图成功（未发送）: " + labels.join("；")) };
  } catch (e) {
    const msg = (e && e.message) ? e.message : String(e);
...
engine.event.subscribe("art.generate", function (ev) {
  const p = ev && ev.payload ? ev.payload : {};
  const result = generatePicture(p, p.context);
  engine.signal.all({ topic: "art.generate", result: result });
  return result;
});

engine.event.subscribe("art.layer_split", function (ev) {
  const p = ev && ev.payload ? ev.payload : {};
  const result = layerSplit(p, p.context);
  engine.signal.all({ topic: "art.layer_split", result: result });
  return result;
});

engine.export("generatePicture", function (params) {
  return generatePicture(params);
});

engine.export("layerSplit", function (params) {
...
function onLoad() {
  ...
  console.info("[绘图] LTP9 绘图插件已加载（同步），后端数量=" + n);
}
function onUnload() { console.info("[绘图] LTP9 绘图插件已卸载"); }