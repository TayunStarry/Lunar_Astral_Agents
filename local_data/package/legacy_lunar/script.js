 ...
 * Live2D 容器面板元素
 */
const live2dContainerPanel = document.getElementById("live2dContainerPanel");
/**
 * 功能控制面板元素
 */
const quickControlPanel = document.getElementById("quickControlPanel");
/**
...
async function EnableLunarToolPackageProtocol() {
    /** 获取文件列表 */
    const listRes = await fetch('/file/list/package/legacy_lunar/package');
    // 检查文件列表响应是否成功
    if (!listRes.ok)
    ...
    /** 批量注册工具 */
    toolFiles.forEach(file => fetchMarkdown(`/file/read/package/legacy_lunar/package/${file.name}`).then(content => registerToolFromMarkdown(content)));
...
async function allowChartRedrawing(type, message) {
     ...
     * 获取图表重绘的Markdown内容
     */
    let markdown = await fetchMarkdown('/file/read/package/legacy_lunar/prompts/chartRedrawing.md');
    // 替换Markdown中的占位符
...
async function prohibitChartRedrawing() {
     ...
     * 获取道歉消息的Markdown内容
     */
    const markdown = await fetchMarkdown('/file/read/package/legacy_lunar/prompts/apologyMessage.md');
    // 若调试模式开启，则渲染< 动态提示词 >
    if (OnlyData.isDebugMode) {
        /**
         * 渲染< 动态提示词 >
...
/** 小屏幕宽度阈值 */
const smallScreenWidthThreshold = 654;
/**
...
async function presetMessage() {
    ...
    /** 获取预设的 Markdown 消息内容 */
    const markdown = await fetchMarkdown('/file/read/package/legacy_lunar/prompts/prohibitMessage.md');
...
async function systemInitializationEvent() {
    ...
    // 异步加载系统提示词
    OnlyData.systemPrompt = await fetchMarkdown('/file/read/package/legacy_lunar/prompts/systemPrompt.md');
    // 异步加载图片描述提示词
    OnlyData.imagePrompt = await fetchMarkdown('/file/read/package/legacy_lunar/prompts/imagePrompt.md');
    // 异步加载视频描述提示词
    OnlyData.videoPrompt = await fetchMarkdown('/file/read/package/legacy_lunar/prompts/videoPrompt.md');
    // 异步加载视频总结提示词
    OnlyData.videoSummaryPrompt = await fetchMarkdown('/file/read/package/legacy_lunar/prompts/videoSummaryPrompt.md');
    // 异步获取文件服务 API 端点
...
async function loadDemoMessage() {
    /** 获取演示消息 */
    const markdown = await fetchMarkdown('/file/read/package/legacy_lunar/prompts/demoMessage.md');
    /** 创建助手消息对象 */
    const assistantMsgObj = await createMessageObject("assistant", '', false);
    // 为助手消息对象设置内容为演示消息
...
function windowResizeEvent() {
    ...
    if (window.innerWidth > smallScreenWidthThreshold) {
        ...
        live2dContainerPanel.style.position = "fixed";
        live2dContainerPanel.style.left = "-9999px";
        live2dContainerPanel.style.top = "-9999px";
        // 显示功能控制面板
        quickControlPanel.style.display = "flex";
        /** 捕获所有配置面板 */
    ...
    // 移除 Live2D 容器面板的样式
    live2dContainerPanel.removeAttribute("style");
    // 隐藏功能控制面板
    quickControlPanel.style.display = "none";
    // 隐藏聊天历史容器面板
    chatHistoryContainerPanel.style.display = "none";
    // 移除调试模式切换按钮的点击状态样式
    debugModeButton.classList.remove("clicking");
...
async function allowActiveMessage() {
     ...
     * 获取主动消息的Markdown内容
     */
    const markdown = await fetchMarkdown('/file/read/package/legacy_lunar/prompts/activeMessage.md');
    // 若调试模式开启，则渲染< 动态提示词 >
    if (OnlyData.isDebugMode) {
        /**
         * 渲染< 动态提示词 >
