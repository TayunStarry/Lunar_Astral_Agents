 ...
 * 全部同步：不使用 async/await/Promise/fetch。网络走同步 API http.get/post（LTP9-Flash 顶层全局）；
 * JWT 走 hash.signJWT；事件订阅与导出函数都返回普通对象（非 Promise）。
 */
...
function getConfig() {
  ...
  return {
    enabled: (cfg.plugin && cfg.plugin.enabled !== undefined) ? cfg.plugin.enabled : true,
    provider: (cfg.api && cfg.api.provider) || "auto",
    qweatherAuth: (cfg.api && cfg.api.qweather_auth) || "api_key",
    qweatherHost: (cfg.api && cfg.api.qweather_host) || "",
    qweatherKey: (cfg.api && cfg.api.qweather_key) || "",
    qweatherProjectID: (cfg.api && cfg.api.qweather_project_id) || "",
    qweatherKeyID: (cfg.api && cfg.api.qweather_key_id) || "",
    qweatherPrivateKey: (cfg.api && cfg.api.qweather_private_key) || "",
    defaultCity: (cfg.display && cfg.display.default_city) || "",
    showAQI: (cfg.display && cfg.display.show_aqi !== undefined) ? cfg.display.show_aqi : true
  };
}
...
function queryQWeather(city) {
  ...
  const headers = {};
  if (useJWT) {
    const now = Math.floor(time.now()) - 30;
    const token = hash.signJWT({ sub: config.qweatherProjectID, iat: now, exp: now + 900 }, config.qweatherPrivateKey, "EdDSA", config.qweatherKeyID);
...
event.subscribe("weather.query", function (ev) {
  const p = ev && ev.payload ? ev.payload : {};
  const result = queryWeather(p.city);
  signal.all({ topic: "weather.query", result: result });
  return result;
});

// 导出函数：同步（非 async）
exportFunction("queryWeather", function (city) {
  return queryWeather(city);
});

// 探针：用于确认引擎执行的是本（同步）版本
exportFunction("ping", function () {
  return { pong: true, src: "com.yaraflow.weather-ltp9", sync: true, version: 4 };
});
...
(function () {
  const cfg = getConfig();
  console.info("[天气] LTP9 天气插件已加载（LTP9-Flash 同步），provider=" + cfg.provider + ", 默认城市=" + (cfg.defaultCity || "无"));
})();