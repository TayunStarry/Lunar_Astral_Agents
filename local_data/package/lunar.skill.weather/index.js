/**
 * 天气查询插件
 * 描述：支持多API提供者自动回退（和风天气 → Open-Meteo → wttr.in）
 * 版本：1.0.0
 * 更新：enumValues支持
 */
...
function ensureConfigFile() {
  ...
  try {
    ...
    if (config && Object.keys(config).length > 0) {
      if (!config.api || !config.api.qweather_host) {
        needsMigration = true;
      }
    }
  } catch (e) {}
...
function getConfig() {
  ...
  yara.logger.info("天气查询: getConfig - 原始配置.api=" + JSON.stringify(config.api));
  if (!config) return {};
...
function queryQWeather(city) {
  ...
  if (useJWT) {
    try {
      yara.logger.info("天气查询: [和风天气] JWT参数检查 - projectID=" + (projectID ? "有" : "无") + ", keyID=" + (keyID ? "有" : "无") + ", privateKey.length=" + privateKey.length + ", privateKey.startsWith=" + privateKey.startsWith("-----BEGIN"));
...
function getWeather(city) {
  ...
  if (config.provider === "auto") {
    ...
    yara.logger.info("天气查询: [auto模式] host=" + (hasHost ? "有" : "无") + ", auth=" + config.qweatherAuth + ", key=" + (hasKeyCreds ? "有" : "无") + ", jwt=" + (hasJWTCreds ? "有" : "无") + ", 启用和风=" + (hasQWeatherCreds ? "是" : "否"));
