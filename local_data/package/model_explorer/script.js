...
function saveConnectionConfig() {
    ...
    localStorage.setItem('model_explorer_config', JSON.stringify(config));
}

// 加载连接配置
function loadConnectionConfig() {
    try {
        const saved = localStorage.getItem('model_explorer_config');
        if (saved) {
...
function resetForm() {
    if (confirm('确定要重置所有配置吗？')) {
        ...
        localStorage.removeItem('model_explorer_config');
