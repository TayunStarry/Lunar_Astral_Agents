...
import { ThirdPersonController } from './core/third-person-control.js';
...
// ==== 常量 ====
// 频道名通过 URL 参数 ?channel=xxx 传入，使各宿主项目可独立通信
const CHANNEL_NAME = urlParams.get('channel') || 'simple-physics-engine';
const SOURCE_ENGINE = 'engine';

// 模型资源路径（通过 URL ?model= 参数指定 fetch 基址，默认指向公共模型目录）
const MODEL_BASE_URL = urlParams.get('model') || '/file/read/models/bedrock/';
// 从 MODEL_BASE_URL 推导 /file/read|write 用的相对路径（去掉 /file/read/ 前缀及尾部斜杠）
const MODEL_FILE_PATH = MODEL_BASE_URL.startsWith('/file/read/')
    ? MODEL_BASE_URL.replace(/^\/file\/read\//, '').replace(/\/$/, '')
    : '';
...
async function init() {
    ...
    // 13.5 第三人称越肩操控模式（按 C 进入/退出；WASD 移动 / 空格跳跃 / 鼠标环绕）
...
function onUpdate(dt) {
    ...
    // 5.5 第三人称越肩操控（WASD 移动 / 空格跳跃 / 鼠标环绕；相机由该模式全权接管）
    thirdPerson?.tick(dt);

    // 6. 相机控制器（非越肩操控模式时驱动；越肩模式由 thirdPerson 直接驱动相机）
    ...
    try {
        ...
        if (manifest.model) {
            ...
            if (modelResp.ok) {
                ...
                if (manifest.texture) {
                    // 月岩引擎模型格式：index_geometry.json（几何）+ index_texture.png（纹理）
                    const parsed = GeometryLoader.parseBedrock(modelJson);
                    parsed.textureRef = { url: baseURL + manifest.texture };
                    await applyModel(parsed);
                } else {
                    // 兼容：旧 .bbmodel 内嵌纹理
...
async function autoLoadImageAssets() {
    try {
        ...
        for (const fileName of configFiles) {
            const resp = await fetch(`/file/read/package/lunar.rendering_engine-studio/property/${fileName}`);
            if (resp.ok) {
...
async function handleAssetSave(id, name) {
    ...
    try {
        ...
        const relativePath = `${MODEL_FILE_PATH}/assets/${safeName}.json`;
...
async function applyModel(result) {
    currentModel = result;
    // 纹理加载：月岩引擎采用 index_geometry.json + index_texture.png 的 Bedrock 运行时格式
    // （result.textureRef 表示外部纹理文件），回退为兼容 .bbmodel 内嵌纹理
    if (result.textureRef?.url) {
        await textureManager.loadFromFile(result.textureRef.url, {
            uvWidth: result.meta?.texWidth,
            uvHeight: result.meta?.texHeight,
            name: result.metadata?.modelIdentifier || 'index_texture',
        });
    } else {
...
// ==== 第三人称越肩操控模式切换 ====
function toggleThirdPerson(enter) {
    if (!thirdPerson) return;
    if (enter) {
        movementController?.cancelMovement?.(); // 清除旧移动目标，避免与新操控冲突
        thirdPerson.enter();
        if (thirdPerson.active) {
            // 进入越肩操控：抑制自动鼠标追踪、退出点击移动
    ...
    broadcast('third_person_changed', { enabled: !!thirdPerson.active });
}
...
function bindKeyboard() {
    window.addEventListener('keydown', (e) => {
        ...
        // 越肩操控模式：独占移动键（WASD/Space/Shift）+ R 重置视角 + C 退出
        if (thirdPerson?.active) {
            if (['KeyW', 'KeyA', 'KeyS', 'KeyD', 'Space', 'ShiftLeft', 'ShiftRight'].includes(code)) {
                e.preventDefault();
                thirdPerson.setKey(code, true);
            } else if (code === 'KeyR') {
                // 重置越肩视角（回到模型正后方、默认距离）
                thirdPerson.yaw = 0;
                thirdPerson.pitch = 1.1;
                thirdPerson.distance = 36;
            } else if (code === 'KeyC') {
...
async function handleChannelMessage(msg) {
    ...
    // 越肩操控模式下，忽略面板/智能体下发的移动、旋转、聚焦指令，避免与新操控冲突
    if (thirdPerson?.active && ['movement', 'rotation', 'focus_character', 'quick_track', 'action'].includes(type)) {
        return;
    }
    ...
    switch (type) {
        ...
        case 'config_save': {
            ...
            if (configType === 'physics' || configType === 'all') {
                saveData.physics = {
                    ...
                    gravity: physicsManager?.gravity ?? -98.2,
                };
            }
            ...
            const writeConfig = async (fileName, data) => {
                const relativePath = `package/lunar.rendering_engine-studio/property/${fileName}.json`;
                const encodedPath = btoa(unescape(encodeURIComponent(relativePath)));
                const resp = await fetch('/file/write', {
                    method: 'POST',
                    headers: {
        ...
        case 'config_load': {
            ...
            const basePath = 'package/lunar.rendering_engine-studio/property';
        ...
        case 'asset_import_scene': {
            ...
            if (!assetId) break;
            try {
                const relativePath = `${MODEL_FILE_PATH}/assets/${assetId}.json`;
...
async function handleAnimGroupSave() {
    ...
    try {
        ...
        const relativePath = `${MODEL_FILE_PATH}/anim_group_config.json`;
        const encodedPath = btoa(unescape(encodeURIComponent(relativePath)));
        const resp = await fetch('/file/write', {
            method: 'POST',
            headers: {
