...
class CameraController {
    ...
    tick(dt) {
        ...
        // 越肩模式：平滑插值焦点到目标位置
        if (!this._osFocus) this._osFocus = new THREE.Vector3();
        this._osFocus.lerp(this._osTarget.position, Math.min(1, dt * 8));

        // 相机相对焦点保持固定偏移
        const offset = new THREE.Vector3(0, this._osHeight, this._osMinDistance + 4);
        const desired = this._osFocus.clone().add(offset);
