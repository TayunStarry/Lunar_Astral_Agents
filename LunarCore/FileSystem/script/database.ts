 ...
 */
export async function getPromptFromDatabase(key: string): Promise<string | null> {
	try {
		/** 定义数据库操作对象数组 */
		...
		/** 解析数据库查询响应 */
		const result: BatchResult = await queryFromDatabase(operations, createTableOperation);
		// 检查查询结果是否有效
 ...
 */
export async function savePromptToDatabase(key: string, prompt: string): Promise<boolean> {
	try {
		/** 检查是否存在相同索引键的记录 */
