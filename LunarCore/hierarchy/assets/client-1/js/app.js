...
async function renderMessage(message, container) {
    ...
    if (message.imageUrl) {
        ...
        img.onclick = () => previewImage(message.imageUrl, message.content || '图片');

        imgContainer.appendChild(img);

        if (message.content) {
            imgContainer.style.setProperty('--image-label', `"${message.content}"`);
        } else {
            imgContainer.style.setProperty('--image-label', '"图片文件"');
        }
...
function removeCodeComments(text) {
    return text
        .replace(/\/\/.*$/gm, '')
        .replace(/\/\*[\s\S]*?\*\//g, '')
        .replace(/'/g, '"');
...
const Live2D = {
    async init() {
        ...
        try {
            ...
            this.setEmotionState(EmotionalState.IDLE);
    ...
    async loadModel() {
        ...
        try {
            if (Live2DModel) {
                pixiJSExample.stage.removeChild(Live2DModel);
                Live2DModel.destroy();
                Live2DModel = null;
            }
            ...
            Live2DModel = await window.PIXI.live2d.Live2DModel.from(
                currentLive2DModel.url,
                { autoInteract: currentLive2DModel.autoInteract }
            );

            Live2DModel.scale.set(currentLive2DModel.scale);
            Live2DModel.anchor.set(0.5, 0.5);

            const container = document.getElementById('live2dContainer');
            Live2DModel.x = (container?.clientWidth || 0) * currentLive2DModel.x;
            Live2DModel.y = (container?.clientHeight || 0) * currentLive2DModel.y;

            pixiJSExample.stage.addChild(Live2DModel);
    ...
    reloadContainer() {
        ...
        if (Live2DModel && currentLive2DModel) {
            const scale = container.clientHeight < 500
                ? currentLive2DModel.scale * 0.65
                : currentLive2DModel.scale;

            Live2DModel.scale.x = scale;
            Live2DModel.scale.y = scale;
            Live2DModel.x = container.clientWidth * currentLive2DModel.x;
            Live2DModel.y = container.clientHeight * currentLive2DModel.y;
        }
    },
...
class LunarCoreApp {
    ...
    async handleWebSocketMessage(message) {
        switch (message.type) {
            ...
            case 'error':
                ...
                this.setLoadingState(false);
                Live2D.setStateWithTimeout(EmotionalState.IDLE);
                break;
        }
    }

    async handleAssistantMessage(content, imageBase64) {
    ...
    async handleSend() {
        ...
        Live2D.setEmotionState(EmotionalState.AWAIT);
