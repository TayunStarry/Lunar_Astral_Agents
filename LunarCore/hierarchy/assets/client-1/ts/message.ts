...
export async function renderMermaid(container: HTMLElement): Promise<void> {
    ...
    for (const block of Array.from(mermaidBlocks)) {
        const textContent = (block as HTMLElement).textContent || '';
...
export function renderECharts(container: HTMLElement): void {
    container.querySelectorAll('.echarts-placeholder').forEach(async (placeholder: any) => {
        try {
...
export function createMessageElement(message: HistoryMessage, container: HTMLElement): HTMLElement {
...
export async function renderMessage(message: HistoryMessage, container: HTMLElement): Promise<HTMLElement> {
    const messageElement = createMessageElement(message, container);
