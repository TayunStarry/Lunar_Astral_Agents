...
/**
 * XMLHttpRequest 补丁 - 修复同步请求中设置 responseType 的问题
 * 某些库（如 pixi-live2d-display）在同步 XHR 请求中设置 responseType 会导致 InvalidAccessError
 * 此补丁将同步请求转换为异步请求以避免该问题
 */
(function() {
    ...
    function PatchedXHR() {
        ...
        const proxy = new Proxy(xhr, {
            get(target, prop) {
                if (prop === 'open') {
                    return function(method, url, async = true, user, password) {
                        isSyncRequest = async === false;
                        openArgs = { method, url, async, user, password };
                        // 强制使用异步模式
                        return target.open(method, url, true, user, password);
                    };
                }
                if (prop === 'responseType') {
                    return pendingResponseType !== null ? pendingResponseType : target.responseType;
                }
                if (prop === 'send') {
                    return function(...args) {
                        // 如果有待设置的 responseType，在 send 之前设置
                        if (pendingResponseType !== null) {
                            try {
                                target.responseType = pendingResponseType;
                            } catch (e) {
                                console.warn('[XHR Patch] Failed to set responseType:', e);
                            }
            ...
            set(target, prop, value) {
                if (prop === 'responseType') {
                    // 延迟设置 responseType 到 send 调用时
                    pendingResponseType = value;
                    return true;
                }
                target[prop] = value;
                return true;
