...
import (
	execute "Lunar-Astral-Agents/library" // 导入执行模块，用于处理核心逻辑
...
type KnowledgeWriteRequest struct {
	FilePath string                 `json:"filePath"`
	Message  execute.HistoryMessage `json:"message"`
}
...
func KnowledgeQueryHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 调用 execute 模块查询知识库
	results, err := execute.QueryKnowledge(req.FilePath, req.QueryVector, req.TopK)
	if err != nil {
...
func KnowledgeWriteHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 调用 execute 模块写入知识库
	hasDuplicate, err := execute.WriteKnowledge(req.FilePath, req.Message)
	if err != nil {
...
func KnowledgeFlushHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 调用 execute 模块刷新知识库
	stats, err := execute.FlushKnowledge(req.FilePath)
	if err != nil {
...
func KnowledgeListHandler(w http.ResponseWriter, r *http.Request) {
	...
	results, err := execute.ListKnowledge(req.FilePath)
	if err != nil {
...
func KnowledgeDeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	hasDuplicate, err := execute.DeleteKnowledge(req.FilePath, req.UUID)
	if err != nil {
