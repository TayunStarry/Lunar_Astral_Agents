...
class Toolchain extends Prompt {
	...
	protected deleteRecords(ids: string[]): void {
		...
		for (const id of uniqueIds) {
			...
			if (error) console.error(`[记忆者] 删除记录 ${id} 失败:`, error);
			else console.log(`[记忆者] 已删除记录 ${id}`);
		}
	}
	...
	protected writeArchive(content: string): void {
		...
		if (error) console.error('[记忆者] 写入档案失败:', error);
		else console.log(`[记忆者] 已写入档案: ${content.slice(0, 60)}...`);
	}
...
/** 记忆者角色 */
export class MemoryRole extends Toolchain {
	/** 档案查询 topK */
	...
	/** 组织历史记录（主入口） */
	public memorizeHistoricalRecords(): void {
