import { OnlyData, modelResponse, ModelBuilder, PostMessage, BaseConfig, ToolCall, InferencePayload, ModelProtocol, AuthHeaders } from '../index';

export class OrganizeRole extends ModelBuilder {
	private organizeTools: ToolCall[] = [
		{
			type: "function",
			function: {
	...
	private executeOrganizeTool(toolCall: NonNullable<modelResponse['body']['choices'][0]['message']['tool_calls']>[0]): string {
	...
	private handleQueryRecords(queryText: string, topK: number): string {
	...
	private handleDeleteRecord(id: string): string {
	...
	private handleStoreRecord(content: string): string {
